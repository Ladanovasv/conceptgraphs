...
class ProgramArgs:
    ...
    # Dataset args
    # Path to config file
    dataconfig_path: Union[str, Path] = "dataconfigs/replica.yaml"
    # Path to base dir of dataset
    dataset_path: Union[str, Path] = "/datasets/Scannet/scans"
    # trajectory to sample from
    sequence: Union[str, List[str]] = "room1"
    ...
    dir_to_save_map: str = "room1-map-test"
...
def run_fusion_and_save_map(args):
    ...
    scenes = ["scene0030_00"]
    for scene in scenes:
        args.dir_to_save_map = scene + "-map"
        # args.odomfile = "/mnt/hdd5/Datasets/Replica/" + scene + "/traj.txt"
        args.feat_dir = scene+"f"
        args.sequence = scene
        ...
        slam = PointFusion(odom="gt", dsratio=1,
                           device=args.device, use_embeddings=True)
        ...
        for idx in trange(len(dataset)):
            ...
            pointclouds, _ = slam.step(
                pointclouds.cpu(), frame_cur.cpu())
