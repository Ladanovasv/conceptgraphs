...
class ProgramArgs:
    ...
    data_dir: Union[str, Path] = "/datasets/Scannet/scans"
...
def main():
    ...
    # scenes = ["scene0030_00", "scene0046_00", "scene0086_00", "scene0222_00",
    #           "scene0435_00", "scene0389_00", "scene0378_00", "scene0011_00"]
    scenes = ["scene0030_00"]

    for scene in scenes:
        args.sequence = scene
        args.save_dir = scene + "f"
        args.dataconfig_path = f"dataconfigs/scannet/{scene}.yaml"
        ...
        model, _, preprocess = open_clip.create_model_and_transforms(
            args.open_clip_model, args.open_clip_pretrained_dataset, cache_dir="../CLIP-ViT-H-14-laion2B-s32B-b79K"
        )
        model.cuda()
        model.eval()
        ...
        for idx in trange(len(dataset)):
            ...
            for maskidx in range(len(masks)):
                _x, _y, _w, _h = tuple(
                    masks[maskidx]["bbox"])  # xywh bounding box
            ...
            for maskidx in range(len(masks)):
                _weighted_feat = softmax_scores[maskidx] * global_feat + (
                    1 - softmax_scores[maskidx]) * feat_per_roi[maskidx]
                _weighted_feat = torch.nn.functional.normalize(
                    _weighted_feat, dim=-1)
                outfeat[roi_nonzero_inds[maskidx][:, 0], roi_nonzero_inds[maskidx]
                        [:, 1]] += _weighted_feat[0].detach().cpu().half()
            ...
            # interpolate is not implemented for float yet in pytorch
            ...
            outfeat = outfeat.permute(0, 2, 3, 1)
