        ...
        for j, (key, value) in enumerate(entry.items()):
            ...
            if isinstance(value, str):
                formatted_value = value.replace(
                    "\\n", "\n").replace("\\t", "\t")
                print('    "{}": "{}"{}'.format(
                    key, formatted_value, terminator))
            else:
    ...
    for idx_obj, obj in tqdm(enumerate(scene_map), total=len(scene_map)):
        ...
        for idx_det in tqdm(idx_most_conf):
            ...
            image_tensor = chat.image_processor.preprocess(
                image_crop, return_tensors="pt")["pixel_values"][0]
            ...
            outputs = chat(query=query, image_features=image_features, image_sizes=[image_crop.size])
...
def get_xyxy_from_mask(mask):
    ...
    if non_zero_indices[0].sum() == 0:
        return (0, 0, 0, 0)
    # Извлекаем минимальное и максимальное значение индексов по осям x и y
            ...
            for componentidx, component in enumerate(components):
                ...
                for u, v in zip(
                    ...
                    if args.llm_type == "gpt4":
                        ...
                        curr_chat_messages = DEFAULT_PROMPT + \
                            str(input_json_str) + "\n Assistant answer:"
                        inputs = {"prompt": curr_chat_messages}
                        outputs = requests.post(url, json=inputs)
                        output_dict["object_relation"] = "FAIL"
                        try:
                            out = eval(outputs.json()[
                                       "text"].replace('\\', ""))
                            print("Answer:")
                            print(out)
                            print("----")
